...
static void parse_coding_byte(int cb, int *types, int nb_args)
{
    int shift = 6;
    int i = 0;
...
static void do_execute_process(vm_t *vm, process_t *proc, int op_idx)
{
    ...
    parse_coding_byte(cb, types, nb_args);
    if (validate_types(op_idx, types, nb_args) != SUCCESS) {
        proc->pc = (proc->pc + compute_skip(types, nb_args, op_idx))
            % MEM_SIZE;
        return;
    }
